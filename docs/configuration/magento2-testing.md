...
If you have {doc}`configured Xdebug <xdebug>`, run Unit tests inside **Debug** console (`roll debug` instead of `roll shell`). The code execution will stop at the breakpoints.
...
If you have {doc}`configured Xdebug <xdebug>`, run Integration tests inside **Debug** console (`roll debug` instead of `roll shell`). The code execution will stop at the breakpoints.
...
You can connect to Chrome session with `roll vnc` command.
